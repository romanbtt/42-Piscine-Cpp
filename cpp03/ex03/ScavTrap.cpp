...
ScavTrap::ScavTrap( std::string name ) : ClapTrap(name), _gateKeeperMode(false)
{
	...
	_energyPoints = _energyPointsInit = 50;
	_attackDamage = _attackDamageInit = 20;
}
...
int	ScavTrap::getHitPointsInit( void )
{
	return _hitPointsInit;
}

int	ScavTrap::getEnergyPointsInit( void )
{
	return _energyPointsInit;
}

int	ScavTrap::getAttackDamageInit( void )
{
